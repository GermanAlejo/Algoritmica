/*----------------------------------------------------------------------
  ...
  History : 2004.05.10 file created from file scan.c
            ...
            2016.04.26 I/O exception creation moved to functions
...
public class Scanner {
  ...
  /*------------------------------------------------------------------*/
  /** Create an 'unterminated string' or 'comment' I/O exception.
   *  @return a 'malformed number' I/O exception.
   *  @since  2016.04.26 (Christian Borgelt) */
  /*------------------------------------------------------------------*/

  private IOException illegalChar (int c)
  { return new IOException("illegal character '"
                          +(char)c +"'" +this.lno()); }
  ...
  private IOException unterminated (boolean com)
  { return new IOException("unterminated "
                       +((com) ? "comment " : "string ") +this.lno()); }
  ...
  {                             /* --- format a string */
    ...
    StringBuilder b;            /* formatted string */
    ...
    b = new StringBuilder("\"");/* start with an opening quote */
    for (i = 0; i < n; i++) {   /* traverse the given string */
